function curveCorrection(){
  ...
  var select_point=BABYLON.GUI.Button.CreateSimpleButton("select point","point");
  var curve=BABYLON.GUI.Button.CreateSimpleButton("curve","curve");
  var clear = BABYLON.GUI.Button.CreateSimpleButton("clear_button","Inspector");
  var calculate_collision = BABYLON.GUI.Button.CreateSimpleButton("calculate_collision_button", "Show Collision");
  var apply=BABYLON.GUI.Button.CreateSimpleButton("apply","Apply");
  let assetsDirectory = "https://neural-hive-3d-modules.s3.ap-south-1.amazonaws.com/demo/common/Assets/";
  let baseDirectory = "https://neural-hive-3d-modules.s3.ap-south-1.amazonaws.com/demo/curveCorrection/"
  ...
  var createDefaultEngine = function() { return new BABYLON.Engine(canvas, true, { preserveDrawingBuffer: true, stencil: true,  disableWebGL2Support: false}); };
    ...
    set.onPointerUpObservable.add(function() {
      ...
      for(let i=0;i<2;i++){
        ...
        BABYLON.SceneLoader.ImportMesh("",baseDirectory+"Assets/3d/after/","Maxillary_gum.glb",scene,function (meshes){
          meshes[1].material = pbr2;
          meshes[1].forceSharedVertices()
        });
        BABYLON.SceneLoader.ImportMesh("",baseDirectory+"Assets/3d/after/","upper_tooth.glb",scene,function (meshes){
          for(let j=0; j<meshes.length; j++){
            meshes[j].material = pbr1;
            meshes[j].forceSharedVertices()
          }
        });
        BABYLON.SceneLoader.ImportMesh("",baseDirectory+"Assets/3d/after/","Mandibular_gum.glb",scene,function (meshes){
          meshes[1].material = pbr2;
          meshes[1].forceSharedVertices()
        });
        BABYLON.SceneLoader.ImportMesh("",baseDirectory+"Assets/3d/after/","lower_tooth.glb",scene,function (meshes){
          for(let j=0; j<meshes.length; j++){
            meshes[j].material = pbr1;
            meshes[j].forceSharedVertices()
          }
...
*/
    ...
    curve.color = "#6495ED";
    curve.thickness = 2;
    curve.background = "#FFFFFF";
    curve.cornerRadius = 8;
    curve.onPointerClickObservable.add(function () {
      console.log('curve', clicks);
      if(pointClick == 0){
        //select_point.color = "red";
        curve.isVisible = false;
        pointClick =1
      }
      else if(pointClick == 1){
        //select_point.color = "green";
        curve.isVisible = true;
        pointClick =0
      }
      /*var points = [];
			var positions = spheres.getVerticesData(BABYLON.VertexBuffer.PositionKind)
			for (var k = 0; k < positions.length; k = k + 3) {
				var v = BABYLON.Vector3.FromArray(positions, k);
				//console.log("pos", v);
			points.push(v);
			//var sphere = BABYLON.Mesh.CreateSphere("sphere1", 5, 2, scene);
			}
			var points1 = [];
			for(var j=0;j<clicks.length;j++){
			points1.push(clicks[j].position);
			}*/
      /*var fullBezier = BABYLON.Curve3.CreateQuadraticBezier(quadraticPoints[0], quadraticPoints[1], quadraticPoints[2], 8);
    for (var k = 1; k < nbControls; k++) {
        var bez = BABYLON.Curve3.CreateQuadraticBezier(quadraticPoints[3 * k], quadraticPoints[3 * k + 1], quadraticPoints[3 * k + 2], 8);
        fullBezier = fullBezier.continue(bez)
    }*/

      //var lines = BABYLON.MeshBuilder.CreateLines("lines", {points: fullBezier.getPoints()}, scene);
      ...
      //var catmullRom = BABYLON.Curve3.CreateCatmullRomSpline(points, 8, true); // for all vertex
      var points1 = [];
      for(var j=0;j<clicks.length;j++){
        //points1.push(clicks[j].position);
      ...
      //rabbit.bakeCurrentTransformIntoVertices();
	...
	//BABYLON.Vector3.TransformCoordinatesToRef(mesh.vertices[i],mesh.mesh.getWorldMatrix(),this.tmpVec);
		...
		let distance = BABYLON.Vector3.Distance(all_points[n],vertices[i]);
		//console.log("dis", distance);
		if(distance < 30){
		console.log("there");
		new_near_points.push(vertices[i]);
		var origin = BABYLON.Mesh.CreateSphere("origin", 4, 0.3, scene);
		origin.position = vertices[i];
		break;
		}
      ...
      for(let n = 0; n<all_points.length; n++){
        ...
        if (index != null) {
          ...
          //var origin = BABYLON.Mesh.CreateSphere("origin", 4, 0.3, scene);
          /*var origin = BABYLON.MeshBuilder.CreateSphere("box", {radius: 0.01}, scene);
		origin.position = worldPos;
		origin.checkcollisions = true;
		origin.ellipsoid = new BABYLON.Vector3(55, 1, 10);*/
      ...
      //var extruded = BABYLON.MeshBuilder.ExtrudeShape("extrudedShape", { shape: mySinus, path: path, sideOrientation: BABYLON.Mesh.DOUBLESIDE }, scene);
      /*var points = [];
	var n = 250; // number of points
	var r = 30; //radius
	for (var i = 0; i < n + 1; i++) {
    ...
    select_point.top = "16%";
    select_point.left = "-43%";
    select_point.width = "130px";
    select_point.height = "40px";
    select_point.color = "#6495ED";
    select_point.thickness = 2;
    select_point.background = "#FFFFFF";
    select_point.cornerRadius = 8;
    select_point.onPointerClickObservable.add(function () {
      console.log("this is selected point");
      if(pointClick == 0){
        //select_point.color = "red";
        select_point.isVisible = false;
        pointClick =1
      }
      else if(pointClick == 1){
        //select_point.color = "green";
        select_point.isVisible = true;
        pointClick =0
      }
      //if(point_
    });
    ...
    apply.onPointerClickObservable.add(function () {
      console.log("this is selected point");
      afterapply = 1;
      for(let i=0;i<4;i++){
      ...
      BABYLON.SceneLoader.ImportMesh("",baseDirectory+"Assets/3d/after/", "tooth.glb", scene, function (meshes) { 
        for(let j=0;j<meshes.length;j++){
          meshes[j].forceSharedVertices();
          meshes[j].material = pbr1;
      ...
      BABYLON.SceneLoader.ImportMesh("",baseDirectory+"Assets/3d/after/", "gums.glb", scene, function (meshes) { 
        meshes[1].forceSharedVertices();
    ...
    var images = new BABYLON.GUI.Image("translate", assetsDirectory+"rot_tool.png");
    images.top = "-30%";
    images.left = "-44%";
    images.width = "50px";
    images.height = "50px";
    ...
    gui.addControl(apply);
    gui.addControl(curve);  
    gui.addControl(select_point);
